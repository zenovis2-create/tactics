...
const EXPECTED_CH04_ORDER := [&"CH04_01", &"CH04_02", &"CH04_03", &"CH04_04", &"CH04_05"]
...
func _run() -> void:
    ...
    if campaign == null:
        push_error("CH04 shell runner could not resolve campaign controller.")
        quit(1)
        return

    campaign._active_chapter_id = &"CH03"
    campaign._active_stage_index = 4
    campaign._current_stage = CH03_FINAL_STAGE
    campaign._enter_chapter_three_camp()
    await process_frame
    await process_frame

    var camp_snapshot: Dictionary = main.get_campaign_state_snapshot()
    if String(camp_snapshot.get("mode", "")) != "camp":
        push_error("Expected CH03 camp before CH04 intro, got %s." % [camp_snapshot.get("mode", "")])
        quit(1)
        return
    ...
    if String(intro_snapshot.get("mode", "")) != "chapter_intro":
        push_error("Expected CH04 chapter intro, got %s." % [intro_snapshot.get("mode", "")])
        quit(1)
        return

    if String(intro_snapshot.get("panel_title", "")).find("CH04") == -1:
        push_error("CH04 intro title did not surface.")
        quit(1)
        return
    ...
    for stage_id in EXPECTED_CH04_ORDER:
        main.advance_campaign_step()
        await process_frame
        await process_frame
        ...
        if StringName(stage_snapshot.get("chapter_id", &"")) != &"CH04":
            push_error("Expected CH04 chapter id, got %s." % [stage_snapshot.get("chapter_id", &"")])
            quit(1)
            return
        ...
        if battle == null or battle.stage_data == null or StringName(battle.stage_data.stage_id) != stage_id:
            push_error("CH04 battle shell did not load %s stage data." % [stage_id])
            quit(1)
            return
        ...
        if stage_id != EXPECTED_CH04_ORDER[-1]:
    ...
    if String(final_snapshot.get("panel_title", "")).find("CH04") == -1:
        push_error("CH04 camp title did not surface.")
        quit(1)
        return
    ...
    print("[PASS] CH04 shell runner reached CH04 intro, CH04_01~05 flow, and CH04 camp handoff.")
    quit(0)
...
func _wait_for_player_phase(battle) -> void:
    ...
    while not _is_battle_finished(battle):
        ...
        safety += 1
        if safety > 180:
            push_error("Timed out waiting for player phase in CH04 shell runner.")
            quit(1)
            return
...
func _play_player_phase(battle) -> void:
    while true:
        ...
        push_error("CH04 shell runner could not find a valid player action.")
        quit(1)
        return
