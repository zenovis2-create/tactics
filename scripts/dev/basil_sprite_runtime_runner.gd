...
const BASIL_DATA = preload("res://data/units/enemy_basil.tres")
...
func _run() -> void:
	for state in ["idle", "move", "attack", "guard"]:
		for alias in ["Basil", "enemy_basil"]:
			if not _assert_frames(alias, state, 8):
				return
	if not await _assert_unit_visual_layer():
		return
	print("[PASS] basil_sprite_runtime_runner validated Basil sprite catalog aliases and Unit visual layer.")
	quit(0)
...
func _assert_unit_visual_layer() -> bool:
	...
	root.add_child(unit)
	unit.setup_from_data(BASIL_DATA)
	await process_frame
	var art_layer: CanvasItem = unit.get_node_or_null("CharacterVisualRoot")
	if art_layer == null:
		return _fail("Basil Unit scene should expose CharacterVisualRoot layer.")
	if not art_layer.visible:
		return _fail("Basil should show CharacterVisualRoot when sprite frames exist.")
	var frames := BattleArtCatalog.load_character_sprite_frames(String(BASIL_DATA.unit_id), "idle")
	if frames.is_empty():
		return _fail("Basil should resolve idle sprite frames from unit_id.")
	if not frames.has(unit.character_sprite.texture):
		return _fail("Basil should render an idle sprite frame instead of token art.")
	if unit.token_art.visible:
		return _fail("Basil should hide generic token art when character sprites exist.")
	unit.queue_free()
	await process_frame
	return true
