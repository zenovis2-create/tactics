...
def main() -> int:
    ...
    ap.add_argument("--move-v01", required=True)
    ap.add_argument("--move-v02", required=True)
    ...
    rows = [
        ...
        ("Move", load_rgba(Path(args.move_v01)), load_rgba(Path(args.move_v02))),
    ...
    draw.text((label_w + gap, 8), "Legacy v01", fill=(48, 48, 48, 255))
    draw.text((label_w + gap * 2 + cell_w, 8), "Layered v02", fill=(48, 48, 48, 255))

    for idx, (label, left_img, right_img) in enumerate(rows):
        y = gap + idx * (title_h + cell_h + gap)
        draw.text((16, y + 8), label, fill=(48, 48, 48, 255))
        left_fit = fit(left_img, cell_w, cell_h)
        right_fit = fit(right_img, cell_w, cell_h)
        board.alpha_composite(left_fit, (label_w + gap, y + title_h))
        board.alpha_composite(right_fit, (label_w + gap * 2 + cell_w, y + title_h))
