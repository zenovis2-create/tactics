...
func _run() -> void:
	...
	if String(main.get_campaign_state_snapshot().get("mode", "")) != "camp":
		return _fail("Campaign save-defeat-title-load runner expected to start from camp mode.")
	...
	if saved_data == null:
		return _fail("Campaign save-defeat-title-load runner expected a loadable manual slot.")
	if saved_data.burden != 7 or saved_data.trust != 5 or saved_data.gold != 876:
		return _fail("Saved camp slot should preserve burden/trust/gold before title load.")

	main.defeat_screen.show_defeat(6)
	await process_frame
	if not bool(main.defeat_screen.get_layout_snapshot().get("visible", false)):
		return _fail("Campaign save-defeat-title-load runner expected the defeat surface to become visible.")
	...
	if not bool(main.title_screen.get_layout_snapshot().get("visible", false)):
		return _fail("Campaign save-defeat-title-load runner expected title screen to become visible.")

	main.title_screen._on_load_pressed()
	await process_frame
	if String(main.save_load_panel.get_layout_snapshot().get("mode", "")) != "load":
		return _fail("Title load button should open SaveLoadPanel in load mode after defeat->title return.")
	...
	print("[PASS] campaign_save_defeat_title_load_runner: camp save survives defeat->title return and reloads into a playable battle state.")
	quit(0)
...
func _assert_loaded_battle_state(main: Node) -> bool:
	...
	if String(snapshot.get("mode", "")) != "battle":
		return _fail("Loading the saved camp slot after defeat->title should return the app to battle mode.")
	if StringName(snapshot.get("current_stage_id", &"")) != &"CH01_02":
		return _fail("Loading the saved camp slot after defeat->title should bootstrap the playable opening stage.")
	...
	if data.burden != 7 or data.trust != 5 or data.gold != 876:
		return _fail("Loaded camp save should preserve saved burden/trust/gold values.")
	if not bool(data.flags.get("flag_resonance_tia", false)):
