...
func _ready() -> void:
	...
	_build_props()
	_build_characters()
	title_label.text = "CH05 Archive Pressure Preview"
	hint_label.text = "Left/Right: switch state   Space: replay"
	summary_label.text = "Goal: validate altar vs lever vs gate-control inside archive pressure."
...
func _build_ground() -> void:
	...
	if texture_a == null or texture_b == null:
		push_warning("Missing archive-pressure ground textures.")
		return
	for x in range(-4, 5):
		for y in range(-2, 3):
			...
			sprite.modulate = Color(0.86, 0.84, 0.8, 0.9 if (x + y) % 2 == 0 else 0.82)
			sprite.scale = Vector2(0.62, 0.62)
			ground_root.add_child(sprite)


func _build_props() -> void:
	_add_prop(ALTAR_PATH, Vector2(430, 36), Vector2(0.4, 0.4))
	_add_prop(LEVER_PATH, Vector2(160, 92), Vector2(2.1, 2.1))
	_add_prop(GATE_CONTROL_PATH, Vector2(602, 88), Vector2(2.55, 2.55))
	_add_prop(FIELD_SWORD_PATH, Vector2(-360, 114), Vector2(0.24, 0.24))
