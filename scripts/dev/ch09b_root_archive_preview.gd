...
const ARCHIVE_LECTERN_PATH := "res://assets/props/archive_lectern_01/runtime/archive_lectern_01_integration_v01.png"
const REVISION_CORE_PATH := "res://assets/props/revision_core_01/runtime/revision_core_01_integration_v01.png"
const TRUTH_DAIS_PATH := "res://assets/props/truth_dais_01/runtime/truth_dais_01_integration_v01.png"
...
const CHARACTER_ORDER := ["serin", "rian", "tia", "enemy_raider", "enemy_skirmisher"]
...
func _ready() -> void:
	...
	_build_props()
	_build_characters()
	title_label.text = "CH09B Root Archive Preview"
	hint_label.text = "Left/Right: switch state   Space: replay"
	summary_label.text = "Goal: validate archive-lectern relief, revision-core pressure, and truth-dais witness inside root-archive space."
...
func _build_ground() -> void:
	...
	if texture_a == null or texture_b == null:
		push_warning("Missing root-archive ground textures.")
		return
	for x in range(-4, 5):
		for y in range(-2, 3):
			...
			sprite.modulate = Color(0.82, 0.8, 0.78, 0.9 if (x + y) % 2 == 0 else 0.82)
			sprite.scale = Vector2(0.62, 0.62)
			ground_root.add_child(sprite)


func _build_props() -> void:
	_add_prop(ARCHIVE_LECTERN_PATH, Vector2(176, 82), Vector2(0.42, 0.42))
	_add_prop(REVISION_CORE_PATH, Vector2(608, 44), Vector2(0.46, 0.46))
	_add_prop(TRUTH_DAIS_PATH, Vector2(416, 20), Vector2(0.34, 0.34))
