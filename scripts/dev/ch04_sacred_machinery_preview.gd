...
const GROUND_TILE_PATH := "res://assets/environment/fortress_tile_01/runtime/fortress_tile_01_integration_v01.png"
...
func _ready() -> void:
	...
	_build_props()
	_build_characters()
	title_label.text = "CH04 Sacred Machinery Preview"
	hint_label.text = "Left/Right: switch state   Space: replay"
	summary_label.text = "Goal: validate altar vs lever vs gate-control inside one sacred-machinery surface."
...
func _build_ground() -> void:
	var texture := _load_runtime_png(GROUND_TILE_PATH)
	if texture == null:
		push_warning("Missing sacred-machinery ground texture.")
		return
	for x in range(-4, 5):
		for y in range(-2, 3):
...
func _build_props() -> void:
	_add_prop(ALTAR_PATH, Vector2(420, 46), Vector2(0.42, 0.42))
	_add_prop(LEVER_PATH, Vector2(170, 88), Vector2(2.2, 2.2))
	_add_prop(GATE_CONTROL_PATH, Vector2(610, 90), Vector2(2.6, 2.6))
