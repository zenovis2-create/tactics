...
const EXPECTED_CH03_ORDER := [&"CH03_01", &"CH03_02", &"CH03_03", &"CH03_04", &"CH03_05"]
...
func _run() -> void:
    ...
    if campaign == null:
        push_error("CH03 shell runner could not resolve campaign controller.")
        quit(1)
        return

    campaign._active_chapter_id = &"CH02"
    campaign._active_stage_index = 4
    campaign._current_stage = CH02_FINAL_STAGE
    ...
    if String(camp_snapshot.get("mode", "")) != "camp":
        push_error("Expected CH02 camp before CH03 intro, got %s." % [camp_snapshot.get("mode", "")])
        quit(1)
        return
    ...
    if String(intro_snapshot.get("mode", "")) != "chapter_intro":
        push_error("Expected CH03 chapter intro, got %s." % [intro_snapshot.get("mode", "")])
        quit(1)
        return

    if String(intro_snapshot.get("panel_title", "")).find("CH03") == -1:
        push_error("CH03 intro title did not surface.")
        quit(1)
        return
    ...
    for stage_id in EXPECTED_CH03_ORDER:
        main.advance_campaign_step()
        await process_frame
        await process_frame
        ...
        if StringName(stage_snapshot.get("chapter_id", &"")) != &"CH03":
            push_error("Expected CH03 chapter id, got %s." % [stage_snapshot.get("chapter_id", &"")])
            quit(1)
            return
        ...
        if stage_id != EXPECTED_CH03_ORDER[-1]:
    ...
    if String(final_snapshot.get("panel_title", "")).find("CH03") == -1:
        push_error("CH03 camp title did not surface.")
        quit(1)
        return
    ...
    if not _party_contains_name(party_details, "Tia"):
        push_error("CH03 camp roster did not include Tia.")
        quit(1)
        return

    var presentation_cards: Array = main.campaign_panel.get_snapshot().get("presentation_cards", [])
    if presentation_cards.is_empty():
        push_error("CH03 camp snapshot did not expose presentation cards.")
        quit(1)
        return
...
func _play_battle_to_victory(battle, stage_id: StringName) -> void:
    ...
    if int(battle.current_phase) != int(battle.BattlePhase.VICTORY):
        push_error("CH03 shell runner battle did not finish in victory for %s." % [stage_id])
        quit(1)
        return
...
func _wait_for_player_phase(battle) -> void:
    ...
    while not _is_battle_finished(battle):
        ...
        safety += 1
        if safety > 180:
            push_error("Timed out waiting for player phase in CH03 shell runner.")
            quit(1)
            return
...
func _play_player_phase(battle) -> void:
    while true:
        ...
        push_error("CH03 shell runner could not find a valid player action.")
        quit(1)
        return
