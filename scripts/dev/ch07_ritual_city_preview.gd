...
const BELL_FRAME_PATH := "res://assets/props/bell_frame_01/runtime/bell_frame_01_integration_v01.png"
const CITY_SEAL_DAIS_PATH := "res://assets/props/city_seal_dais_01/runtime/city_seal_dais_01_integration_v01.png"
...
func _ready() -> void:
	...
	_build_props()
	_build_characters()
	title_label.text = "CH07 Ritual City Preview"
	hint_label.text = "Left/Right: switch state   Space: replay"
	summary_label.text = "Goal: validate bell-frame warning pressure and city-seal authority inside ritual-city space."
...
func _build_ground() -> void:
	...
	if texture_a == null or texture_b == null:
		push_warning("Missing ritual-city ground textures.")
		return
	for x in range(-4, 5):
		for y in range(-2, 3):
			...
			sprite.modulate = Color(0.93, 0.9, 0.97, 0.9 if (x + y) % 2 == 0 else 0.82)
			sprite.scale = Vector2(0.62, 0.62)
			ground_root.add_child(sprite)


func _build_props() -> void:
	_add_prop(BELL_FRAME_PATH, Vector2(182, 16), Vector2(0.5, 0.5))
	_add_prop(CITY_SEAL_DAIS_PATH, Vector2(610, 48), Vector2(0.46, 0.46))
