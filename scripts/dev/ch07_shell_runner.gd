...
const CH06_FINAL_STAGE = preload("res://data/stages/ch06_05_stage.tres")
const EXPECTED_CH07_ORDER := [&"CH07_01", &"CH07_02", &"CH07_03", &"CH07_04", &"CH07_05"]
...
func _run() -> void:
    ...
    if campaign == null:
        push_error("CH07 shell runner could not resolve campaign controller.")
        quit(1)
        return

    campaign._active_chapter_id = &"CH06"
    campaign._active_stage_index = 4
    campaign._current_stage = CH06_FINAL_STAGE
    campaign._enter_chapter_six_camp()
    await process_frame
    await process_frame

    var camp_snapshot: Dictionary = main.get_campaign_state_snapshot()
    if String(camp_snapshot.get("mode", "")) != "camp":
        push_error("Expected CH06 camp before CH07 intro, got %s." % [camp_snapshot.get("mode", "")])
        quit(1)
        return
    ...
    if String(intro_snapshot.get("mode", "")) != "chapter_intro":
        push_error("Expected CH07 chapter intro, got %s." % [intro_snapshot.get("mode", "")])
        quit(1)
        return

    if String(intro_snapshot.get("panel_title", "")).find("CH07") == -1:
        push_error("CH07 intro title did not surface.")
        quit(1)
        return

    if String(intro_snapshot.get("panel_body", "")).find("Ellyor") == -1:
        push_error("CH07 intro body did not mention Ellyor.")
        quit(1)
        return

    for stage_id in EXPECTED_CH07_ORDER:
        main.advance_campaign_step()
        await process_frame
        await process_frame

        var stage_snapshot: Dictionary = main.get_campaign_state_snapshot()
        if stage_id == &"CH07_05" and String(stage_snapshot.get("mode", "")) == "choice":
            main.campaign_controller._make_choice("ch07_believe_mira")
            await process_frame
            await process_frame
        ...
        if StringName(stage_snapshot.get("chapter_id", &"")) != &"CH07":
            push_error("Expected CH07 chapter id, got %s." % [stage_snapshot.get("chapter_id", &"")])
            quit(1)
            return
        ...
        if battle == null or battle.stage_data == null or StringName(battle.stage_data.stage_id) != stage_id:
            push_error("CH07 battle shell did not load %s stage data." % [stage_id])
            quit(1)
            return
    ...
    if String(final_snapshot.get("panel_title", "")).find("CH07") == -1:
        push_error("CH07 camp title did not surface.")
        quit(1)
        return

    var panel_body: String = String(final_snapshot.get("panel_body", ""))
    if panel_body.find("black-hound orders") == -1 and panel_body.find("Black-hound orders") == -1:
        push_error("CH07 camp body did not mention the black-hound orders.")
        quit(1)
        return

    if panel_body.find("forest") == -1 and panel_body.find("Forest") == -1:
        push_error("CH07 camp body did not point back toward the forest.")
        quit(1)
        return

    var presentation_cards: Array = main.campaign_panel.get_snapshot().get("presentation_cards", [])
    if presentation_cards.is_empty():
        push_error("CH07 camp snapshot did not expose presentation cards.")
        quit(1)
        return

    if String(presentation_cards[0].get("title", "")).find("Black-Hound") == -1:
        push_error("CH07 camp presentation cards did not expose the black-hound orders.")
        quit(1)
        return

    print("[PASS] CH07 shell runner reached CH07 intro, CH07_01~05 flow, and CH07 camp handoff.")
    quit(0)
...
func _play_battle_to_victory(battle, stage_id: StringName) -> void:
    ...
    if int(battle.current_phase) != int(battle.BattlePhase.VICTORY):
        push_error("CH07 shell runner battle did not finish in victory for %s." % [stage_id])
        quit(1)
        return
...
func _wait_for_player_phase(battle) -> void:
    ...
    while not _is_battle_finished(battle):
        ...
        safety += 1
        if safety > 180:
            push_error("Timed out waiting for player phase in CH07 shell runner.")
            quit(1)
            return
...
func _play_player_phase(battle) -> void:
    while true:
        ...
        push_error("CH07 shell runner could not find a valid player action.")
        quit(1)
        return
