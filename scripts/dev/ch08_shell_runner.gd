...
const EXPECTED_CH08_ORDER := [&"CH08_01", &"CH08_02", &"CH08_03", &"CH08_04", &"CH08_05"]
...
func _run() -> void:
    ...
    if campaign == null:
        push_error("CH08 shell runner could not resolve campaign controller.")
        quit(1)
        return

    campaign._active_chapter_id = &"CH07"
    campaign._active_stage_index = 4
    campaign._current_stage = CH07_FINAL_STAGE
    campaign._enter_chapter_seven_camp()
    await process_frame
    await process_frame

    var camp_snapshot: Dictionary = main.get_campaign_state_snapshot()
    if String(camp_snapshot.get("mode", "")) != "camp":
        push_error("Expected CH07 camp before CH08 intro, got %s." % [camp_snapshot.get("mode", "")])
        quit(1)
        return
    ...
    if String(intro_snapshot.get("mode", "")) != "chapter_intro":
        push_error("Expected CH08 chapter intro, got %s." % [intro_snapshot.get("mode", "")])
        quit(1)
        return

    if String(intro_snapshot.get("panel_title", "")).find("CH08") == -1:
        push_error("CH08 intro title did not surface.")
        quit(1)
        return
    ...
    for stage_id in EXPECTED_CH08_ORDER:
        ...
        if battle == null or battle.stage_data == null or StringName(battle.stage_data.stage_id) != stage_id:
            push_error("CH08 battle shell did not load %s stage data." % [stage_id])
            quit(1)
            return

        if not _has_cutscene_start(battle.cutscene_player.get_event_log(), _get_stage_intro_cutscene_id(stage_id)):
            push_error("CH08 shell did not start intro cutscene %s." % [_get_stage_intro_cutscene_id(stage_id)])
            quit(1)
            return
        ...
        if not _has_cutscene_start(victory_cutscene_entries, _get_stage_outro_cutscene_id(stage_id)):
            push_error("CH08 shell did not start clear cutscene %s." % [_get_stage_outro_cutscene_id(stage_id)])
            quit(1)
            return

        var post_battle_snapshot: Dictionary = main.get_campaign_state_snapshot()
        if stage_id != EXPECTED_CH08_ORDER[-1]:
    ...
    if String(final_snapshot.get("panel_title", "")).find("CH08") == -1:
        push_error("CH08 camp title did not surface.")
        quit(1)
        return
    ...
    if presentation_cards.is_empty():
        push_error("CH08 camp snapshot did not expose presentation cards.")
        quit(1)
        return
...
func _play_battle_to_victory(battle, stage_id: StringName) -> void:
    ...
    if int(battle.current_phase) != int(battle.BattlePhase.VICTORY):
        push_error("CH08 shell runner battle did not finish in victory for %s." % [stage_id_text])
        quit(1)
        return
...
func _wait_for_player_phase(battle) -> void:
    ...
    while not _is_battle_finished(battle):
        ...
        safety += 1
        if safety > 180:
            push_error("Timed out waiting for player phase in CH08 shell runner.")
            quit(1)
            return
...
func _play_player_phase(battle) -> void:
    while true:
        ...
        push_error("CH08 shell runner could not find a valid player action.")
        quit(1)
        return
...
func _take_action_for_unit(battle, unit) -> bool:
    ...
    if interaction_destination != Vector2i(-1, -1):
        ...
        # Keep the shell runner focused on interaction objectives once it has
        # committed movement toward them. Falling back into combat AI here can
        # undo authored routing assumptions and stall chapter-specific stages.
