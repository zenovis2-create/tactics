...
func _run() -> void:
	for state in ["idle", "move", "attack", "guard"]:
		for alias in ["Scout", "ally_scout"]:
			if not _assert_frames(alias, state, 8):
				return
	if not await _assert_unit_visual_layer():
		return
	print("[PASS] scout_sprite_runtime_runner validated Scout sprite catalog aliases and Unit visual layer.")
	quit(0)
...
func _assert_unit_visual_layer() -> bool:
	...
	root.add_child(unit)
	unit.setup_from_data(SCOUT_DATA)
	await process_frame
	...
	if not art_layer.visible:
		return _fail("Scout should show CharacterVisualRoot when sprite frames exist.")
	var frames := BattleArtCatalog.load_character_sprite_frames(String(SCOUT_DATA.display_name), "idle")
	if frames.is_empty():
		return _fail("Scout should resolve idle sprite frames from display name.")
	if unit.character_sprite.texture != frames[0]:
		return _fail("Scout should render the first idle sprite frame instead of token art.")
	if unit.token_art.visible:
		return _fail("Scout should hide generic token art when character sprites exist.")
	unit.queue_free()
	await process_frame
	return true
