...
const EXPECTED_CH05_ORDER := [&"CH05_01", &"CH05_02", &"CH05_03", &"CH05_04", &"CH05_05"]
...
func _run() -> void:
    ...
    if campaign == null:
        push_error("CH05 shell runner could not resolve campaign controller.")
        quit(1)
        return

    campaign._active_chapter_id = &"CH04"
    campaign._active_stage_index = 4
    campaign._current_stage = CH04_FINAL_STAGE
    campaign._enter_chapter_four_camp()
    await process_frame
    await process_frame

    var camp_snapshot: Dictionary = main.get_campaign_state_snapshot()
    if String(camp_snapshot.get("mode", "")) != "camp":
        push_error("Expected CH04 camp before CH05 intro, got %s." % [camp_snapshot.get("mode", "")])
        quit(1)
        return
    ...
    if String(intro_snapshot.get("mode", "")) != "chapter_intro":
        push_error("Expected CH05 chapter intro, got %s." % [intro_snapshot.get("mode", "")])
        quit(1)
        return

    if String(intro_snapshot.get("panel_title", "")).find("CH05") == -1:
        push_error("CH05 intro title did not surface.")
        quit(1)
        return

    if String(intro_snapshot.get("panel_body", "")).find("Archive") == -1 and String(intro_snapshot.get("panel_body", "")).find("archive") == -1:
        push_error("CH05 intro body did not mention the archive.")
        quit(1)
        return

    for stage_id in EXPECTED_CH05_ORDER:
        main.advance_campaign_step()
        await process_frame
        await process_frame

        var stage_snapshot: Dictionary = main.get_campaign_state_snapshot()
        if stage_id == &"CH05_05" and String(stage_snapshot.get("mode", "")) == "choice":
            main.campaign_controller._make_choice("ch05_save_ledgers")
            await process_frame
            await process_frame
        ...
        if StringName(stage_snapshot.get("chapter_id", &"")) != &"CH05":
            push_error("Expected CH05 chapter id, got %s." % [stage_snapshot.get("chapter_id", &"")])
            quit(1)
            return
        ...
        if battle == null or battle.stage_data == null or StringName(battle.stage_data.stage_id) != stage_id:
            push_error("CH05 battle shell did not load %s stage data." % [stage_id])
            quit(1)
            return
        ...
        if stage_id != EXPECTED_CH05_ORDER[-1]:
    ...
    if String(final_snapshot.get("panel_title", "")).find("CH05") == -1:
        push_error("CH05 camp title did not surface.")
        quit(1)
        return

    var panel_body: String = String(final_snapshot.get("panel_body", ""))
    if panel_body.find("Enoch joins") == -1:
        push_error("CH05 camp body did not mention Enoch joining.")
        quit(1)
        return

    if panel_body.find("Valtor") == -1:
        push_error("CH05 camp body did not point toward Valtor.")
        quit(1)
        return

    var party_details: Array = main.campaign_panel.get_snapshot().get("party_details", [])
    if not _party_contains_name(party_details, "Enoch"):
        push_error("CH05 camp roster did not include Enoch.")
        quit(1)
        return

    var presentation_cards: Array = main.campaign_panel.get_snapshot().get("presentation_cards", [])
    if presentation_cards.is_empty():
        push_error("CH05 camp snapshot did not expose presentation cards.")
        quit(1)
        return

    if String(presentation_cards[0].get("title", "")).find("Enoch") == -1:
        push_error("CH05 camp presentation cards did not expose Enoch's handoff.")
        quit(1)
        return

    print("[PASS] CH05 shell runner reached CH05 intro, CH05_01~05 flow, and CH05 camp handoff.")
    quit(0)
...
func _play_battle_to_victory(battle, stage_id: StringName) -> void:
    ...
    if int(battle.current_phase) != int(battle.BattlePhase.VICTORY):
        push_error("CH05 shell runner battle did not finish in victory for %s." % [stage_id])
        quit(1)
        return
...
func _wait_for_player_phase(battle) -> void:
    ...
    while not _is_battle_finished(battle):
        ...
        safety += 1
        if safety > 180:
            push_error("Timed out waiting for player phase in CH05 shell runner.")
            quit(1)
            return
...
func _play_player_phase(battle) -> void:
    while true:
        ...
        push_error("CH05 shell runner could not find a valid player action.")
        quit(1)
        return
