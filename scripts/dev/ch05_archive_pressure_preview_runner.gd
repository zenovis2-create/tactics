...
const PREVIEW_SCENE: PackedScene = preload("res://scenes/dev/CH05ArchivePressurePreview.tscn")
...
func _run() -> void:
	...
	print("[PASS] ch05_archive_pressure_preview_runner validated archive-pressure preview loading.")
	quit(0)
...
func _assert_ground_loaded(scene: Node) -> bool:
	...
	if ground_root == null:
		return _fail("CH05 archive pressure preview should expose GroundRoot.")
	if ground_root.get_child_count() <= 0:
		return _fail("CH05 archive pressure preview should spawn ground sprites.")
	var textured := 0
	for child in ground_root.get_children():
		if child is Sprite2D and child.texture != null:
			textured += 1
	if textured == 0:
		return _fail("CH05 archive pressure ground sprites should resolve textures.")
	return true
...
func _assert_prop_mix_loaded(scene: Node) -> bool:
	...
	if prop_root == null:
		return _fail("CH05 archive pressure preview should expose PropRoot.")
	var textured := 0
	for child in prop_root.get_children():
		if child is Sprite2D and child.texture != null:
			textured += 1
	if textured < 4:
		return _fail("CH05 archive pressure preview should load altar, lever, gate-control, and field-sword props.")
	return true
...
func _assert_character_layer_loaded(scene: Node) -> bool:
	...
	if character_root == null:
		return _fail("CH05 archive pressure preview should expose CharacterRoot.")
	...
	if animated_count < 5:
		return _fail("CH05 archive pressure preview should load five animated characters.")
	return true
