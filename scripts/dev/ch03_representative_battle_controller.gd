...
func bootstrap_battle() -> void:
	...
	if vanguard != null and is_instance_valid(vanguard):
		vanguard.set_grid_position(Vector2i(3, 7), stage_data.cell_size)
	if scout != null and is_instance_valid(scout):
		scout.set_grid_position(Vector2i(6, 7), stage_data.cell_size)
	if enemy_units.size() >= 2:
		var raider = enemy_units[0]
		var skirmisher = enemy_units[1]
		if raider != null and is_instance_valid(raider):
			raider.set_grid_position(Vector2i(8, 3), stage_data.cell_size)
	...
	if vanguard != null and is_instance_valid(vanguard):
		...
		hud.set_action_hint("Survey the trail markers to map Greenwood's safe route.")
