...
const CHARACTER_ORDER := ["rian", "serin", "tia", "enemy_raider", "enemy_skirmisher"]
...
func _ready() -> void:
	...
	_build_props()
	_build_characters()
	title_label.text = "CH03 Forest Trap Preview"
	hint_label.text = "Left/Right: switch state   Space: replay"
	summary_label.text = "Goal: validate shrine vs trap vs route-control in a Greenwood ambush surface."
...
func _build_ground() -> void:
	...
	if texture_a == null or texture_b == null:
		push_warning("Missing forest-trap ground textures.")
		return
	for x in range(-4, 5):
		for y in range(-2, 3):
			...
			sprite.modulate = Color(0.92, 0.96, 0.9, 0.92 if (x + y) % 2 == 0 else 0.84)
			sprite.scale = Vector2(0.62, 0.62)
			ground_root.add_child(sprite)


func _build_props() -> void:
	_add_prop(ALTAR_PATH, Vector2(430, 42), Vector2(0.38, 0.38))
	_add_prop(LEVER_PATH, Vector2(165, 92), Vector2(2.05, 2.05))
	_add_prop(GATE_CONTROL_PATH, Vector2(615, 86), Vector2(2.35, 2.35))
