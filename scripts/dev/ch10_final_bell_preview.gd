...
const ANCHOR_CHAIN_PATH := "res://assets/props/anchor_chain_01/runtime/anchor_chain_01_integration_v01.png"
const BELL_DAIS_PATH := "res://assets/props/bell_dais_01/runtime/bell_dais_01_integration_v01.png"
...
func _ready() -> void:
	...
	_build_props()
	_build_characters()
	title_label.text = "CH10 Final Bell Preview"
	hint_label.text = "Left/Right: switch state   Space: replay"
	summary_label.text = "Goal: validate anchor-chain suppression and bell-dais terminal ritual pressure without shared objective drift."
...
func _build_ground() -> void:
	...
	if texture_a == null or texture_b == null:
		push_warning("Missing final-bell ground textures.")
		return
	for x in range(-4, 5):
		for y in range(-2, 3):
			...
			sprite.modulate = Color(0.78, 0.79, 0.88, 0.9 if (x + y) % 2 == 0 else 0.82)
			sprite.scale = Vector2(0.62, 0.62)
			ground_root.add_child(sprite)

func _build_props() -> void:
	_add_prop(ANCHOR_CHAIN_PATH, Vector2(190, 66), Vector2(0.54, 0.54))
	_add_prop(BELL_DAIS_PATH, Vector2(610, 54), Vector2(0.48, 0.48))
...
func _play_current_animation() -> void:
	...
	for key_variant in CHARACTER_ORDER:
		...
		if not animations.has(resolved_animation):
			resolved_animation = "idle"
		sprite.play(resolved_animation)
